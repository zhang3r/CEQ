...
public class SoccerGame implements DomainGenerator {
	...
	List<PropositionalFunction> generatePFs() {
		return Arrays.asList(new AgentInUGoal(PF_IN_U_GOAL), new AgentInPGoal(
				PF_IN_P_GOAL));
	}
	...
	public OOSGDomain generateDomain() {
		...
		domain.setJointActionModel(new SoccerMechanics(domain,
				this.semiWallProb));
	...
	public static State getSimpleGameInitialState() {
		...
		GenericOOState s = new GenericOOState(
				new SoccerAgent(0, 0, 0, "agent0"), new SoccerAgent(2, 0, 1,
						"agent1"), new GGGoal(0, 2, 1, "g0"), new GGGoal(2, 2,
						2, "g1"));
	...
	public static State getCorrdinationGameInitialState() {
		...
		GenericOOState s = new GenericOOState(
				new SoccerAgent(0, 0, 0, "agent0"), new SoccerAgent(2, 0, 1,
						"agent1"), new GGGoal(0, 2, 2, "g0"), new GGGoal(2, 2,
						1, "g1"));
	...
	public static State getPrisonersDilemmaInitialState() {
		...
		GenericOOState s = new GenericOOState(
				new SoccerAgent(3, 0, 0, "agent0"), new SoccerAgent(5, 0, 1,
						"agent1"), new GGGoal(0, 0, 1, "g0"), new GGGoal(4, 0,
						0, "g1"), new GGGoal(8, 0, 2, "g2"));
	...
	public static State getFriendFoeInitialState() {
		...
		GenericOOState s = new GenericOOState(
				new SoccerAgent(3, 0, 0, "agent0"), new SoccerAgent(6, 0, 1,
						"agent1"), new GGGoal(0, 0, 1, "g0"), new GGGoal(4, 0,
						0, "g1"));
	...
	public static State getIncredibleInitialState() {
		...
		GenericOOState s = new GenericOOState(
				new SoccerAgent(2, 0, 0, "agent0"), new SoccerAgent(3, 0, 1,
						"agent1"), new GGGoal(0, 0, 1, "g0"), new GGGoal(1, 0,
						2, "g1"));
	...
	public static State getTurkeyInitialState() {
		...
		GenericOOState s = new GenericOOState(
				new SoccerAgent(0, 0, 0, "agent0"), new SoccerAgent(2, 0, 1,
						"agent1"), new GGGoal(0, 3, 1, "g0"), new GGGoal(1, 2,
						0, "g1"), new GGGoal(2, 3, 2, "g2"),
	...
	/**
	 * /** Sets boundary walls of a domain. This method will add 4 solid walls
	 * (top left bottom right) to create a playing field in which the agents can
	 * interact.
	 * 
	 * @param s
	...
	public static void setBoundaryWalls(GenericOOState s, int w, int h) {
		...
		s.addObject(new GGWall.GGVerticalWall(0, h - 1, 0, 0, "h" + numH))
				.addObject(
						new GGWall.GGVerticalWall(0, h - 1, w, 0, "h"
								+ (numH + 1)))
				.addObject(
						new GGWall.GGHorizontalWall(0, w - 1, 0, 0, "v" + numV))
				.addObject(
						new GGWall.GGHorizontalWall(0, w - 1, h, 0, "v"
								+ (numV + 1)));

	}

	/**
	 * Creates and returns a standard {@link SGAgentType} for grid games. This
	 * {@link SGAgentType} is assigned the type name "agent", grid game OO-MDP
	 * object class for "agent", and has its action space set to all possible
	 * actions in the grid game domain. Typically, all agents in a grid game
	 * should be assigned to the same type.
	 *
	 * @param domain
	 *            the domain object of the grid game.
	 * @return An {@link SGAgentType} that typically all {@link SGAgent}'s of
	 *         the grid game should play as.
	 */
	public static SGAgentType getStandardSoccerGameAgentType(SGDomain domain) {
		return new SGAgentType(SoccerGame.CLASS_AGENT, domain.getActionTypes());
	}
	...
	static class AgentInPGoal extends PropositionalFunction {
		...
		public boolean isTrue(OOState s, String... params) {
			...
			boolean hasBall = (Boolean) agent.get("hasBall");
	...
	public static class GGJointRewardFunction implements JointRewardFunction {
		...
		public GGJointRewardFunction(OODomain ggDomain) {
			agentInPersonalGoal = ggDomain.propFunction(SoccerGame.PF_IN_P_GOAL);
			agentInUniversalGoal = ggDomain.propFunction(SoccerGame.PF_IN_U_GOAL);
		}
		 ...
		 */
		public GGJointRewardFunction(OODomain ggDomain, double stepCost,
		...
		public double[] reward(State s, JointAction ja, State sp) {
			...
			List<ObjectInstance> obs = osp.objectsOfClass(SoccerGame.CLASS_AGENT);
			...
			List<GroundedProp> upgps = agentInUniversalGoal
					.allGroundings((OOState) sp);
			for (GroundedProp gp : upgps) {
				String agentName = gp.params[0];
				if (gp.isTrue(osp)) {
					int aid = ((SoccerAgent) ((OOState) sp).object(agentName)).player;
			...
			for (GroundedProp gp : ipgps) {
				String agentName = gp.params[0];
				ObjectInstance agent = ((OOState) s).object(agentName);
				boolean hasBall = ((SoccerAgent) agent).isHasBall();
				if (gp.isTrue(osp) && hasBall) {
					int aid = ((SoccerAgent) ((OOState) sp).object(agentName)).player;
		...
		protected double defaultCost(int aid, JointAction ja) {
			...
			} else if (ja.action(aid) == null
					|| ja.action(aid).actionName().equals(SoccerGame.ACTION_NOOP)) {
				return 0.;
			}
			return this.stepCost;
		}
		...
		protected double getPersonalGoalReward(OOState s, String agentName) {
			...
			int pn = (Integer) s.object(agentName).get(SoccerGame.VAR_PN);
	...
	public static class GGTerminalFunction implements TerminalFunction {
		...
		public boolean isTerminal(State s) {
			...
			for (GroundedProp gp : ipgps) {
				...
				boolean hasBall = ((SoccerAgent) ((OOState) s)
						.object(agentName)).isHasBall();
				if (gp.isTrue((OOState) s) && hasBall) {
					return true;
				}
			}
	...
	public static void main(String[] args) {
		...
		State s = SoccerGame.getTurkeyInitialState();
