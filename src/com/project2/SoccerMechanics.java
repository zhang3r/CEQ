 ...
 *
 */
public class SoccerMechanics implements FullJointModel {
	 ...
	 */
	public SoccerMechanics(Domain d){
	 ...
	 */
	public SoccerMechanics(Domain d, double semiWallPassThroughProb){
		rand = RandomFactory.getMapped(0);
		domain = d;
	...
	public List<StateTransitionProb> stateTransitions(State s, JointAction ja) {
		...
		List <Location2> previousLocations = new ArrayList<SoccerMechanics.Location2>();
		List <Location2> noopLocations = new ArrayList<SoccerMechanics.Location2>();

		int pn = 0;
		for(Action gsa : ja){
			Location2 loc = this.getLocation((OOState)s, this.agentName(pn, (OOState)s));
			previousLocations.add(loc);
			if(gsa.actionName().equals(SoccerGame.ACTION_NOOP)){
		...
		for(LocationSetProb sp : outcomeSets){
			...
			for(LocationSetProb csp : cOutcomeSets){
				...
				for(int i = 0; i < csp.locs.size(); i++){
					...
					((MutableState)agent).set(SoccerGame.VAR_X, loc.x);
					((MutableState)agent).set(SoccerGame.VAR_Y, loc.y);
					((MutableState)agent).set("hasBall", !(Boolean)((MutableState)agent).get("hasBall"));
	...
	public State sample(State s, JointAction ja) {
		...
		List <Location2> basicMoveResults = new ArrayList<SoccerMechanics.Location2>();
		for(int i = 0; i < ja.size(); i++){
			Location2 loc = previousLocations.get(i);
			Action gsa = ja.action(i);
	 ...
	 * @return a {@link SoccerMechanics.Location2} object containing the agents position in the world.
	 */
	protected Location2 getLocation(OOState s, String agentName){
		
		ObjectInstance a = s.object(agentName);
		Location2 loc = new Location2((Integer)a.get(SoccerGame.VAR_X), (Integer)a.get(SoccerGame.VAR_Y));
	...
	protected Location2 attemptedDelta(String actionName){
		...
		if(actionName.equals(SoccerGame.ACTION_NORTH)){
			return new Location2(0, 1);
		}
		else if(actionName.equals(SoccerGame.ACTION_SOUTH)){
			return new Location2(0, -1);
		}
		else if(actionName.equals(SoccerGame.ACTION_EAST)){
			return new Location2(1, 0);
		}
		else if(actionName.equals(SoccerGame.ACTION_WEST)){
			return new Location2(-1, 0);
		}
		else if(actionName.equals(SoccerGame.ACTION_NOOP)){
			return new Location2(0, 0);
		}
		
		throw new RuntimeException("Error: Unknown action named '" + actionName + "' that SoccerMechanics cannot handle");
	}
	...
	protected List<Location2> resolvePositionSwaps(List <Location2> originalPositions, List<Location2> desiredPositions){
		...
		List<Location2> resolvedPositions = new ArrayList<SoccerMechanics.Location2>(desiredPositions);
		List <Location2> newNoopPositions = new ArrayList<SoccerMechanics.Location2>();
	...
	protected List <Location2> backupNoOps(List <Location2> originalPositions, List<Location2> desiredPositions, List <Location2> noops){
		...
		List <Location2> resolved = new ArrayList<SoccerMechanics.Location2>(desiredPositions);
	...
	protected List<Location2> resolveCollisions(List<Location2> originalPositions, List <Location2> desiredPositions){
		...
		List <Location2> finalPoses = new ArrayList<SoccerMechanics.Location2>();
	...
	protected List <LocationSetProb> getPossibleCollisionOutcomes(List<Location2> originalPositions, List <Location2> desiredPositions){
		...
		if(collissionSets.size() == 0){
			...
			List <LocationSetProb> outcomes = new ArrayList<SoccerMechanics.LocationSetProb>(1);
			outcomes.add(new LocationSetProb(new ArrayList<SoccerMechanics.Location2>(desiredPositions), 1.));
			return outcomes;
			
		}
		
		List <LocationSetProb> allOutcomes = new ArrayList<SoccerMechanics.LocationSetProb>();
	...
	protected Location2 sampleBasicMovement(OOState s, Location2 p0, Location2 delta, List <Location2> agentNoOpLocs){
		...
		if(delta.x != 0 && !reset){
			reset = this.sampleWallCollision(p0, delta, s.objectsOfClass(SoccerGame.CLASS_DIM_V_WALL), true);
		}
		
		if(delta.y != 0 && !reset){
			reset = this.sampleWallCollision(p0, delta, s.objectsOfClass(SoccerGame.CLASS_DIM_H_WALL), false);
		}
	...
	protected List <Location2Prob> getPossibleLocationsFromWallCollisions(OOState s, Location2 p0, Location2 delta, List <Location2> agentNoOpLocs){
		...
		List <Location2Prob> locs = new ArrayList<SoccerMechanics.Location2Prob>(2);
		...
		if(delta.x != 0){
			int wc = this.wallCollision(p0, delta, s.objectsOfClass(SoccerGame.CLASS_DIM_V_WALL), true);
			if(wc == 0){
			...
			int wc = this.wallCollision(p0, delta, s.objectsOfClass(SoccerGame.CLASS_DIM_H_WALL), false);
			if(wc == 0){
				locs.add(new Location2Prob(p1, 1.)); //agent freely moves
			}
			else if(wc == 1){
	...
	protected boolean sampleWallCollision(Location2 p0, Location2 delta, List <ObjectInstance> walls, boolean vertical){
		...
		for(int i = 0; i < walls.size(); i++){
			...
			if(this.crossesWall(p0, delta, w, vertical)){
				int wt = (Integer)w.get(SoccerGame.VAR_WT);
				if(wt == 0){ //solid wall
					return true;
				}
	...
	protected boolean crossesWall(Location2 p0, Location2 delta, ObjectInstance w, boolean vertical){
		...
		int wp = (Integer)w.get(SoccerGame.VAR_POS);
		int we1 = (Integer)w.get(SoccerGame.VAR_E1);
		int we2 = (Integer)w.get(SoccerGame.VAR_E2);
	...
	protected List<LocationSetProb> getAllLocationSets(List<List<Location2Prob>> locOutcomes){
		...
		List <LocationSetProb> sets = new ArrayList<SoccerMechanics.LocationSetProb>();
	...
	protected void generateAllLocationSetsHelper(List<List<Location2Prob>> locOutcomes, int i, Location2 [] locArray, double stackedProb, List<LocationSetProb> discovered){
		...
		if(i == locOutcomes.size()){
			//bundle them up
			List <Location2> locs = new ArrayList<SoccerMechanics.Location2>(locArray.length);
	...
	protected boolean agentsEqual(OOState s1, OOState s2){
		...
		List<ObjectInstance> agents1 = s1.objectsOfClass(SoccerGame.CLASS_AGENT);
		for(ObjectInstance a1 : agents1){
			ObjectInstance a2 = s2.object(a1.name());
			
			int x1 = (Integer)a1.get(SoccerGame.VAR_X);
			int x2 = (Integer)a2.get(SoccerGame.VAR_X);
			
			int y1 = (Integer)a1.get(SoccerGame.VAR_Y);
			int y2 = (Integer)a2.get(SoccerGame.VAR_Y);
	...
	protected String agentName(int agentNum, OOState s){
		for(ObjectInstance o : s.objectsOfClass(SoccerGame.CLASS_AGENT)){
			int opn = (Integer)o.get(SoccerGame.VAR_PN);
			if(opn == agentNum){
				return o.name();
			}
		}
